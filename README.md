...
| 字型精準度 | Cap-height 基準線對齊、Font 0 內嵌 BarlowCondensed-Bold 字型並以 per-character advance width 精確對齊 ZPL 標準輸出、四方向旋轉 |
...
| 標籤設定 | 可調整寬度、高度（**mm**）與解析度（203 / 300 / 600 DPI） |
...
│       │   └── util/
│       │       └── CharWidthMeasurer.java  # 從參考 PNG 量測字元 advance width
│       ├── main/resources/
...
```properties
# 條碼間最小水平間距（mm），0 = 停用檢查
zpl.barcode.min-horizontal-gap-mm=2.5

# 安全邊界（mm），各邊可獨立設定
zpl.margin.top-mm=1
zpl.margin.bottom-mm=1
zpl.margin.left-mm=3
zpl.margin.right-mm=8

# 重疊判定閾值（mm），兩欄位交集的寬 AND 高都超過此值才視為重疊（0 = 任何交集均觸發）
zpl.overlap.threshold-mm=0
```
...
| `width` | double | 152.0 | 標籤寬度（**mm**） |
| `height` | double | 102.0 | 標籤高度（**mm**） |
...
任一欄位的邊界超出安全區域即觸發，並標示超出的邊（LEFT／TOP／RIGHT／BOTTOM）及超出量（mm）。安全邊界由 `application.properties` 獨立設定各邊：

| 屬性 | 說明 |
|------|------|
| `zpl.margin.top-mm` | 上邊界安全距離（mm） |
| `zpl.margin.bottom-mm` | 下邊界安全距離（mm） |
| `zpl.margin.left-mm` | 左邊界安全距離（mm） |
| `zpl.margin.right-mm` | 右邊界安全距離（mm） |

### 重疊（OVERLAP）

對所有欄位兩兩比對交集矩形。當交集的**寬度 AND 高度**同時超過 `zpl.overlap.threshold-mm`（`application.properties`）時才觸發警告，避免因浮點誤差或刻意設計（如邊框）產生誤報。

### 條碼間距不足（BARCODE_GAP）

對所有條碼欄位兩兩比對水平間距。當兩個條碼在 Y 軸有重疊且水平間距小於 `zpl.barcode.min-horizontal-gap-mm`（預設 2.5 mm）時觸發警告，防止印表機讀取錯誤。設為 0 可停用此項檢查。
...
- **量測方式**：使用 `CharWidthMeasurer` 工具，從 ZPL 參考 PNG（`^A0N,30,24`）偵測各字元的像素起始位置，計算 advance width（`segs[i+1].start − segs[i].start`），轉換成 `ratio = dots / fontHeight`
...
```bash
cd backend
mvn exec:java "-Dexec.mainClass=com.zplviewer.util.CharWidthMeasurer" "-Dexec.args=../image/font_reference.png"
```
...
- 標籤尺寸換算：`dots = mm × dpmm`（例如 102 mm × 8 dpmm = 816 dots）
